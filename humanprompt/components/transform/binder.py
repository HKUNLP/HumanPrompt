...
class BinderTransform(Transform):
    @staticmethod
    def transform(
            x: Union[str, Dict[str, Union[str, Dict]]], y: Union[str, Dict] = None, **kwargs: Any
    ) -> str:
        """
        ...
        db_prompt = build_db_prompt(table, title, prompt_style) if prompt_style else build_db_prompt(table, title)
        ...
        if y is None:
            ...
            return f"{db_prompt}{qa_prompt}{y['nsql']}"
